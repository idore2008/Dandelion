...
(function() {
	...
	window.KW = {
		...
		toggleDisplay : function(id) { // -----显示，隐藏-----
			...
			oTarget.style.display == 'none' ? oTarget.style.display = 'block'
		...
		geterateCompareTRs : function(iCol, sDataType) { // 比较函数，用于sort排序用
			return function compareTRs(oTR1, oTR2) {
				...
				if (oTR1.cells[iCol].getAttribute("value")) { // 用于高级排序，比如图片，添加一个额外的属性来排序
					vValue1 = KW.convert(
							oTR1.cells[iCol].getAttribute("value"), sDataType);
					vValue2 = KW.convert(
							oTR2.cells[iCol].getAttribute("value"), sDataType);
				} else {
					vValue1 = KW.convert(oTR1.cells[iCol].firstChild.nodeValue,
							sDataType);
					vValue2 = KW.convert(oTR2.cells[iCol].firstChild.nodeValue,
							sDataType);
				}
				if (vValue1 < vValue2) {
					return -1;
		...
		ajax : function(obj) {
			...
			XHR.onreadystatechange = function() {
				if (XHR.readyState == 4
					...
					if (obj.success) {
						if (dataType && dataType.toLocaleLowerCase() === "json") {
							obj.success(eval("(" + XHR.responseText + ")"))
								...
								&& dataType.toLocaleLowerCase() === "xml") {
					...
					if (obj.error) {
						obj.error("The XMLHttpRequest failed. status: "
								+ XHR.status);
					}
